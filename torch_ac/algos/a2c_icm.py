...
class A2CAlgoICM(A2CAlgo):
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.01, gae_lambda=0.95,
                 ...
                 rmsprop_alpha=0.99, rmsprop_eps=1e-8, preprocess_obss=None, ir_coeff=0.01, reshape_reward=None):
...
#initialize intrinsic rewards
        ...
        self.embedding_network=  MinigridStateEmbeddingNet(envs[0].observation_space)
        self.forward_dynamics_model= MinigridForwardDynamicsNet(envs[0].observation_space, envs[0].action_space)
    ...
    #         state_emb = self.embedding_network(sb.obs[:][:-1]) 
    #         next_state_emb = self.embedding_network(sb.obs[:][1:]) 
    #         pred_next_state_emb= self.forward_dynamics_model(state_emb,sb.action[:-1])
    #         self.intrinsic_rewards = torch.norm(pred_next_state_emb - next_state_emb, dim=1, p=2)
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            if i==self.num_frames_per_proc-1: #include the first frame and the next state after the last frame
        ...
        exps.obs_to_embed = self.preprocess_obss(exps.obs_to_embed)#, device=self.device)
        ...
        self.intrinsic_rewards= torch.norm(pred_next_state_emb - next_state_emb, dim=1, p=2).to('cuda:5')
    ...
    def update_parameters(self, exps):
        ...
        for i in range(self.recurrence):
            ...
            loss = policy_loss - self.entropy_coef * entropy + self.value_loss_coef * value_loss + sb.forward_dynamics_loss
            #print('loss',loss)
            # Update batch values
        ...
        update_grad_norm = sum(p.grad.data.norm(2) ** 2 for p in self.acmodel.parameters()) ** 0.5
        torch.nn.utils.clip_grad_norm_(self.acmodel.parameters(), self.max_grad_norm)
        torch.nn.utils.clip_grad_norm(self.forward_dynamics_model.parameters(),self.max_grad_norm)
        self.optimizer.step()
